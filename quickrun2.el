...
(defun quickrun2--without-focus ()
  (let ((quickrun2-focus-p nil))
    (quickrun2)))

;;;###autoload
(define-minor-mode quickrun2-autorun-mode
  "`quickrun2' after saving buffer"
  :init-value nil
  :global nil
  :lighter " QAR"
  (if quickrun2-autorun-mode
      (add-hook 'after-save-hook 'quickrun2--without-focus nil t)
    (remove-hook 'after-save-hook 'quickrun2--without-focus t)))

;;
;; helm/anything interface
;;

(defconst helm-quickrun2--actions
  '(("Run this cmd-key" . quickrun2--helm-action-default)
    ("Compile only" . quickrun2--helm-compile-only)
    ("Run with shell" . quickrun2--helm-action-shell)
    ("Run with argument" . quickrun2--helm-run-with-arg)
    ("Replace region" . quickrun2--helm-action-replace-region)
    ("Eval and insert as comment" . quickrun2--helm-action-eval-print)))

(defvar helm-quickrun2-source
  `((name . "Choose Command-Key")
    (volatile)
    (candidates . (lambda ()
                    (cl-loop for (cmd-key . cmd-info) in quickrun2--language-alist
                             collect (quickrun2--helm-candidate cmd-key cmd-info))))
    (action . ,helm-quickrun2--actions))
  "helm/anything source of `quickrun2'")

(defvar quickrun2--helm-history nil)

(defvar helm-quickrun2-history-source
  `((name . "Helm Quickrun2 History")
    (volatile)
    (candidates . quickrun2--helm-history)
    (action . ,helm-quickrun2--actions))
  "helm source of `quickrun2' history")

(defun quickrun2--helm-candidate (cmd-key cmd-info)
  (let ((description (or (assoc-default :description cmd-info) "")))
    (cons (format "%-25s %s" cmd-key description) cmd-key)))

(defun quickrun2--helm-update-history (cmd-key)
  (setq quickrun2--helm-history
        (cons cmd-key
              (delete cmd-key quickrun2--helm-history))))

(defun quickrun2--helm-action-default (cmd-key)
  (quickrun2--helm-update-history cmd-key)
  (let ((quickrun2-option-cmdkey cmd-key)
        start end)
    (if (use-region-p)
        (setq start (region-beginning) end (region-end))
      (setq start (point-min) end (point-max)))
    (quickrun2-region start end)))

(defun quickrun2--helm-run-with-arg (cmd-key)
  (quickrun2--helm-update-history cmd-key)
  (let ((quickrun2-option-cmdkey cmd-key))
    (call-interactively 'quickrun2-with-arg)))

(defun quickrun2--helm-action-shell (cmd-key)
  (quickrun2--helm-update-history cmd-key)
  (let ((quickrun2-option-cmdkey cmd-key))
    (quickrun2-shell)))

(defun quickrun2--helm-compile-only (cmd-key)
  (quickrun2--helm-update-history cmd-key)
  (let ((quickrun2-option-cmdkey cmd-key))
    (quickrun2-compile-only)))

(defun quickrun2--helm-action-replace-region (cmd-key)
  (quickrun2--helm-update-history cmd-key)
  (let ((quickrun2-option-cmdkey cmd-key))
    (quickrun2-replace-region (region-beginning) (region-end))))

(defun quickrun2--helm-action-eval-print (cmd-key)
  (quickrun2--helm-update-history cmd-key)
  (let ((quickrun2-option-cmdkey cmd-key))
    (quickrun2-eval-print (region-beginning) (region-end))))

;;;###autoload
(defun anything-quickrun2 ()
  (interactive)
  (unless (featurep 'anything)
    (error "anything is not installed."))
  (anything helm-quickrun2-source))

;;;###autoload
(defun helm-quickrun2 ()
  (interactive)
  (unless (featurep 'helm)
    (error "helm is not installed."))
  (let ((sources (if quickrun2--helm-history
                     '(helm-quickrun2-history-source helm-quickrun2-source)
                   '(helm-quickrun2-source))))
    (helm :sources sources :buffer "*helm quickrun2*")))
